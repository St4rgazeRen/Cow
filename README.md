...
service/
  ...
  market_data.py      BTC / DXY 歷史數據（五層備援：本地DB→Yahoo→Binance→Kraken→CryptoCompare）+ T 日數據縫合（縫合結果即時寫回 CSV，避免重複抓取）
...
- **feat(app)**: 引入 `streamlit-autorefresh`，每 60 秒自動重整頁面，確保 BTC 即時價格持續更新（配合 `fetch_realtime_data` TTL=60s）。
